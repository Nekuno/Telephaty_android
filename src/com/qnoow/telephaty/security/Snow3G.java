package com.qnoow.telephaty.security;




public class Snow3G{

/* LFSR */

long LFSR_S0 = 0x00;
long LFSR_S1 = 0x00;
long LFSR_S2 = 0x00;
long LFSR_S3 = 0x00;
long LFSR_S4 = 0x00;
long LFSR_S5 = 0x00;
long LFSR_S6 = 0x00;
long LFSR_S7 = 0x00;
long LFSR_S8 = 0x00;
long LFSR_S9 = 0x00;
long LFSR_S10 = 0x00;
long LFSR_S11 = 0x00;
long LFSR_S12 = 0x00;
long LFSR_S13 = 0x00;
long LFSR_S14 = 0x00;
long LFSR_S15 = 0x00;

/* FSM */

long FSM_R1 = 0x00;
long FSM_R2 = 0x00;
long FSM_R3 = 0x00;

/* Rijndael S-box SR */

private long SR[] = {

};

/* S-box SQ */

private long SQ[] = {

};

/* MULx.
* Input V: an 8-bit input.
* Input c: an 8-bit input.
* Output : an 8-bit output.
*/

private long MULx(long V, long c)
{
	if ( (V & 0x80) != -1 )
		return ( (V << 1) ^ c);
	else
		return ( V << 1);
}

/* MULxPOW.
* Input V: an 8-bit input.
* Input i: a positive longer.
* Input c: an 8-bit input.
* Output : an 8-bit output.
*/

private long MULxPOW(long V, long i, long c)
{
	if ( i == 0)
		return V;
	else
		return MULx( MULxPOW( V, i-1, c ), c);
}

/* The function MUL alpha.
* Input c: 8-bit input.
* Output : 32-bit output.
*/

private long MULalpha(long c)
{
	return ( ( ((long)MULxPOW(c, 23, 0xa9)) << 24 ) |
		( ((long)MULxPOW(c, 245, 0xa9)) << 16 ) |
		( ((long)MULxPOW(c, 48, 0xa9)) << 8 ) |
		( ((long)MULxPOW(c, 239, 0xa9)) ) ) ;
}

/* The function DIV alpha.
* Input c: 8-bit input.
* Output : 32-bit output.
*/

private long DIValpha(long c)
{
	return ( ( ((long)MULxPOW(c, 16, 0xa9)) << 24 ) |
		( ((long)MULxPOW(c, 39, 0xa9)) << 16 ) |
		( ((long)MULxPOW(c, 6, 0xa9)) << 8 ) |
		( ((long)MULxPOW(c, 64, 0xa9)) ) ) ;
}

/* The 32x32-bit S-Box S1
* Input: a 32-bit input.
* Output: a 32-bit output of S1 box.
*/

private long S1(long w)
{
	long r0=0, r1=0, r2=0, r3=0;
	long srw0 = SR[ (int)((w >> 24) & 0xff) ];
	long srw1 = SR[ (int)((w >> 16) & 0xff) ];
	long srw2 = SR[ (int)((w >> 8) & 0xff) ];
	long srw3 = SR[ (int)((w) & 0xff) ];
	r0 = ( ( MULx( srw0 , 0x1b) ) ^
		( srw1 ) ^
		( srw2 ) ^
		( (MULx( srw3, 0x1b)) ^ srw3 )
	);
	r1 = ( ( ( MULx( srw0 , 0x1b) ) ^ srw0 ) ^
		( MULx(srw1, 0x1b) ) ^
		( srw2 ) ^
		( srw3 )
	);
	r2 = ( ( srw0 ) ^
		( ( MULx( srw1 , 0x1b) ) ^ srw1 ) ^
		( MULx(srw2, 0x1b) ) ^
		( srw3 )
	);
	r3 = ( ( srw0 ) ^
		( srw1 ) ^
		( ( MULx( srw2 , 0x1b) ) ^ srw2 ) ^
		( MULx( srw3, 0x1b) )
	);

	return ( ( ((long)r0) << 24 ) | ( ((long)r1) << 16 ) | ( ((long)r2) << 8 ) |
		( ((long)r3) ) );
}

/* The 32x32-bit S-Box S2
* Input: a 32-bit input.
* Output: a 32-bit output of S2 box.
*/

private long S2(long w)
{
	long r0=0, r1=0, r2=0, r3=0;
	long sqw0 = SQ[ (int)((w >> 24) & 0xff) ];
	long sqw1 = SQ[ (int)((w >> 16) & 0xff) ];
	long sqw2 = SQ[ (int)((w >> 8) & 0xff) ];
	long sqw3 = SQ[ (int)((w) & 0xff) ];
	r0 = ( ( MULx( sqw0 , 0x69) ) ^
		( sqw1 ) ^
		( sqw2 ) ^
		( (MULx( sqw3, 0x69)) ^ sqw3 )
	);
	r1 = ( ( ( MULx( sqw0 , 0x69) ) ^ sqw0 ) ^
		( MULx(sqw1, 0x69) ) ^
		( sqw2 ) ^
		( sqw3 )
	);
	r2 = ( ( sqw0 ) ^
		( ( MULx( sqw1 , 0x69) ) ^ sqw1 ) ^
		( MULx(sqw2, 0x69) ) ^
		( sqw3 )
	);
	r3 = ( ( sqw0 ) ^
		( sqw1 ) ^
		( ( MULx( sqw2 , 0x69) ) ^ sqw2 ) ^
		( MULx( sqw3, 0x69) )
	);
	return ( ( ((long)r0) << 24 ) | ( ((long)r1) << 16 ) | ( ((long)r2) << 8 ) |
		( ((long)r3) ) );
}

/* Clocking LFSR in initialization mode.
* LFSR Registers S0 to S15 are updated as the LFSR receives a single clock.
* Input F: a 32-bit word comes from output of FSM.
*/

private void ClockLFSRInitializationMode(long F)
{
	long v = ( ( (LFSR_S0 << 8) & 0xffffff00 ) ^
		( MULalpha( (long)((LFSR_S0>>24) & 0xff) ) ) ^
		( LFSR_S2 ) ^
		( (LFSR_S11 >> 8) & 0x00ffffff ) ^
		( DIValpha( (long)( ( LFSR_S11) & 0xff ) ) ) ^
		( F )
	);
	LFSR_S0 = LFSR_S1;
	LFSR_S1 = LFSR_S2;
	LFSR_S2 = LFSR_S3;
	LFSR_S3 = LFSR_S4;
	LFSR_S4 = LFSR_S5;
	LFSR_S5 = LFSR_S6;
	LFSR_S6 = LFSR_S7;
	LFSR_S7 = LFSR_S8;
	LFSR_S8 = LFSR_S9;
	LFSR_S9 = LFSR_S10;
	LFSR_S10 = LFSR_S11;
	LFSR_S11 = LFSR_S12;
	LFSR_S12 = LFSR_S13;
	LFSR_S13 = LFSR_S14;
	LFSR_S14 = LFSR_S15;
	LFSR_S15 = v;
}

/* Clocking LFSR in keystream mode.
* LFSR Registers S0 to S15 are updated as the LFSR receives a single clock.
*/

private void ClockLFSRKeyStreamMode()
{
	long v = ( ( (LFSR_S0 << 8) & 0xffffff00 ) ^
		( MULalpha( (long)((LFSR_S0>>>24) & 0xff) ) ) ^
		( LFSR_S2 ) ^
		( (LFSR_S11 >> 8) & 0x00ffffff ) ^
		( DIValpha( (long)( ( LFSR_S11) & 0xff ) ) )
	);
	LFSR_S0 = LFSR_S1;
	LFSR_S1 = LFSR_S2;
	LFSR_S2 = LFSR_S3;
	LFSR_S3 = LFSR_S4;
	LFSR_S4 = LFSR_S5;
	LFSR_S5 = LFSR_S6;
	LFSR_S6 = LFSR_S7;
	LFSR_S7 = LFSR_S8;
	LFSR_S8 = LFSR_S9;
	LFSR_S9 = LFSR_S10;
	LFSR_S10 = LFSR_S11;
	LFSR_S11 = LFSR_S12;
	LFSR_S12 = LFSR_S13;
	LFSR_S13 = LFSR_S14;
	LFSR_S14 = LFSR_S15;
	LFSR_S15 = v;
}

/* Clocking FSM.
* Produces a 32-bit word F.
* Updates FSM registers R1, R2, R3.
*/

private long ClockFSM()
{
	long F = ( ( LFSR_S15 + FSM_R1 ) & 0xffffffff ) ^ FSM_R2 ;
	long r = ( FSM_R2 + ( FSM_R3 ^ LFSR_S5 ) ) & 0xffffffff ;
	FSM_R3 = S2(FSM_R2);
	FSM_R2 = S1(FSM_R1);
	FSM_R1 = r;
	return F;
}

/* Initialization.
* Input k[4]: Four 32-bit words making up 128-bit key.
* Input IV[4]: Four 32-bit words making 128-bit initialization variable.
* Output: All the LFSRs and FSM are initialized for key generation.
*/

public void Initialize(long k[], long IV[])
{
	long i=0;
	long F = 0x0;
	LFSR_S15 = k[3] ^ IV[0];
	LFSR_S14 = k[2];
	LFSR_S13 = k[1];
	LFSR_S12 = k[0] ^ IV[1];
	LFSR_S11 = k[3] ^ 0xffffffff;
	LFSR_S10 = k[2] ^ 0xffffffff ^ IV[2];
	LFSR_S9 = k[1] ^ 0xffffffff ^ IV[3];
	LFSR_S8 = k[0] ^ 0xffffffff;
	LFSR_S7 = k[3];
	LFSR_S6 = k[2];
	LFSR_S5 = k[1];
	LFSR_S4 = k[0];
	LFSR_S3 = k[3] ^ 0xffffffff;
	LFSR_S2 = k[2] ^ 0xffffffff;
	LFSR_S1 = k[1] ^ 0xffffffff;
	LFSR_S0 = k[0] ^ 0xffffffff;
	FSM_R1 = 0x0;
	FSM_R2 = 0x0;
	FSM_R3 = 0x0;
	for(i=0;i<32;i++)
	{
		F = ClockFSM();
		ClockLFSRInitializationMode(F);
	}
}

/* Generation of Keystream.
* input n: number of 32-bit words of keystream.
* input z: space for the generated keystream, assumes
* memory is allocated already.
* output: generated keystream which is filled in z
*/

public void GenerateKeystream(int n, long ks[])
{
	int t = 0;
	long F = 0x0;
	ClockFSM(); /* Clock FSM once. Discard the output. */
	ClockLFSRKeyStreamMode(); /* Clock LFSR in keystream mode once. */
	for ( t=0; t<n; t++)
	{
		F = ClockFSM(); /* STEP 1 */
		ks[t] = F ^ LFSR_S0; /* STEP 2 */
		/* Note that ks[t] corresponds to z_{t+1} in section 4.2
		*/
		ClockLFSRKeyStreamMode(); /* STEP 3 */
	}
}

}

